"""
Interactive RAG with detailed output format.
Shows: user query → top 5 chunks → prompt sent to LLM → LLM response

Usage:
  # Qwen on HPC
  python src/interactive_openai.py --model "hf:/home/blparne/p24is012/models/Qwen--Qwen2.5-7B-Instruct"

  # Mistral on HPC
  python src/interactive_openai.py --model "hf:/home/blparne/p24is012/models/mistralai--Mistral-7B-Instruct-v0.3"

  # Ollama locally
  python src/interactive_openai.py --model "ollama:gpt-oss:20b"

  # OpenAI
  python src/interactive_openai.py --model "openai:gpt-4"
"""
...
# Import from the RAG pipeline
from src.serve_query import (
    get_index_and_model,   # ← cached loader
...
def interactive_loop(model_choice, k, min_sim, cutoff, strict):
    ...
    # ── Load index + embed model once (cached) ────────────────────────────
    print("[INFO] Loading index and embedding model...")
    index, metas, embed_model = get_index_and_model()
    print("[INFO] Ready.\n")

    print("🚀 Interactive RAG is ready.")
    print(f"   Model : {model_choice}")
    print(f"   Top-k : {k}")
    print(f"   Mode  : {'DEFENSE' if strict else 'ATTACK/NORMAL'}")
    ...
    while True:
        ...
        for i, r in enumerate(retrieved[:5], 1):
            ...
            print(f"  Snippet    : {pretty_snippet(meta.get('chunk_text', ''), 200)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        default="hf:/home/blparne/p24is012/models/Qwen--Qwen2.5-7B-Instruct",
        help="Model to use: hf:<path>, ollama:<name>, openai:<name>"
    )
